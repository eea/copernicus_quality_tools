...
def subtract_table(cursor, ident_colname, first_table, second_table):
    ...
    sql.format(first_table, second_table, ident_colname)
    cursor.execute(sql)
...
def subtract_border_polygons(cursor, status_layer_name, ident_colname, layer_name, error_table_name, except_table_name):
    ...
    sql = ("WITH"
           ...
           "  FROM {3:s} lt, {4:s} et, boundary INNER JOIN lt.{2:s} = et.{2:s}"
           "  WHERE ST_Intersects(lt.wkb_geometry, boundary.wkb_geometry);")
    sql = sql.format(status_layer_name,
                     except_table_name,
                     ident_colname,
                     layer_name,
                     error_table_name)
...
def subtract_inner_polygons(cursor, ident_colname, layer_name, error_table_name, except_table_name, code_colname, area_ha):
    ...
    sql = ("WITH"
           "  all_dissolved AS ("
           "    SELECT ST_Dump(ST_Union(wkb_geometry)) geom FROM {0:s} GROUP BY {1:s}),"
           "  big_dissolved AS ("
           "    SELECT geom FROM all_dissolved WHERE ST_Area(geom) > %s)"
           "INSERT INTO {2:s}"
           "  SELECT lt.{3:s}"
           "  FROM {0:s} lt, {4:s} et, big_dissolved INNER JOIN lt.{3:s} = et.{3:s}"
...
def run_check(params, status):
    ...
    for layer_name in params["db_layer_names"]:
        ...
        if not border_exception:
            ...
            boundary_table_name =  "{:s}_boundary".format(layer_name)
