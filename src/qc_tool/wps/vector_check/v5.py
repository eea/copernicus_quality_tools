...
def run_check(params, status):
    ...
    for layer_info in params["layer_aliases"].values():
        for unique_key in params["unique_keys"]:
            error_table_name = "{:s}_{:s}_uniqueid_error".format(layer_info["pg_layer_name"], unique_key)
            sql = SQL.format(error_table_name, unique_key, layer_info["pg_layer_name"])
            cursor.execute(sql)
            if cursor.rowcount == 0:
                ...
                failed_message = "The column {:s}.{:s} has non-unique values: {:s}.".format(layer_info["pg_layer_name"], unique_key, failed_items_message)
