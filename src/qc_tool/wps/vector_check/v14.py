...
def run_check(params, status):
    ...
    for layer_info in params["layer_aliases"].values():
        ...
        error_table_name = "{:s}_neighbcode_error".format(layer_info["pg_layer_name"])
        error_count = create_all_breaking_neighbcode(cursor, layer_info["pg_fid_name"], layer_info["pg_layer_name"], error_table_name, code_colnames, exclude_codes)
        if error_count == 0:
            cursor.execute("DROP TABLE {:s};".format(error_table_name))
        else:
            failed_pairs_message = get_failed_pairs_message(cursor, error_table_name, layer_info["pg_fid_name"])
            failed_message = "The layer {:s} has neighbouring polygons with the same codes in rows: {:s}.".format(layer_info["pg_layer_name"], failed_pairs_message)
