...
SQL = ("CREATE TABLE {0:s} AS"
       ...
       "  WHERE"
       "    {3:s} IS NULL"
       "    OR {3:s} NOT IN (SELECT code FROM v6_code WHERE data_type=%s);")
...
def run_check(params, status):
    ...
    for layer_info in params["layer_aliases"].values():
        ...
        for column_name, value_set_name in column_defs:
            error_table_name = "{:s}_{:s}_validcodes_error".format(layer_info["pg_layer_name"], column_name)
            ...
            if cursor.rowcount == 0:
                ...
                failed_message = "The layer {:s} has column {:s} with invalid codes in rows: {:s}.".format(layer_info["pg_layer_name"], column_name, failed_items_message)
