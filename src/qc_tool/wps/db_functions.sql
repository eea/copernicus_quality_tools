CREATE FUNCTION custom_functions.__f_count_rows(_tbl regclass, OUT _ct bigint)
RETURNS bigint
LANGUAGE plpgsql
AS $$
BEGIN
...
CREATE FUNCTION custom_functions.__v11_mmu_polyline_border(product_name text)
RETURNS text
LANGUAGE plpgsql
AS $$
DECLARE
...
CREATE FUNCTION custom_functions.__v11_mmu_status(mmu integer, product_name text, product_border boolean, OUT _ct bigint)
RETURNS bigint
LANGUAGE plpgsql
AS $$
DECLARE
...
CREATE FUNCTION custom_functions.__v11_mmu_change(mmu integer, product_name text, product_type text, product_border boolean, code_from text, code_to text)
RETURNS text
LANGUAGE plpgsql
AS $$
DECLARE
