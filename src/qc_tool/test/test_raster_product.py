...
class Test_Raster(ProductTestCase):
    def show_messages(self, job_status):
        ...
        """
        checks_not_ok = [check for check in job_status["checks"] if check["status"] not in ("ok", "skipped")]
        msg = ""
    ...
    def test_fty_020m(self):
        ...
        self.expected_check_statuses["r11"] = "ok"
        ...
        self.assertDictEqual(self.expected_check_statuses, check_statuses, self.show_messages(job_status))
    ...
    def test_gra_020m(self):
        ...
        self.expected_check_statuses["r3"] = "failed"
        job_status = dispatch(self.job_uuid, self.username, filepath, product_ident, self.check_idents)
        check_statuses = dict((check_status["check_ident"], check_status["status"])
                              for check_status in job_status["checks"])
        self.assertDictEqual(self.expected_check_statuses, check_statuses, self.show_messages(job_status))
    ...
    def test_waw_020m(self):
        ...
        self.expected_check_statuses["r10"] = "cancelled"
        job_status = dispatch(self.job_uuid, self.username, filepath, product_ident, self.check_idents)
        check_statuses = dict((check_status["check_ident"], check_status["status"])
                              for check_status in job_status["checks"])
        self.assertDictEqual(self.expected_check_statuses, check_statuses)
