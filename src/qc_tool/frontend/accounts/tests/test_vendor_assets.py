...
class BoundaryUploadClientTests(TestCase):
    ...
    def test_page_uses_one_small_same_origin_upload_client(self):
        ...
        self.assertContains(response, "dashboard/js/features/boundaries/upload.js", count=1)
        self.assertContains(response, "Upload and activate", count=1)
        self.assertContains(response, 'name="csrfmiddlewaretoken"')
        self.assertContains(response, 'data-upload-list')
        self.assertNotContains(response, 'id="modal-progress"')
        self.assertNotContains(response, 'class="progress-bar"')
