    ...
    ui.createQueue = function(root, options) {
        ...
        function refresh() {
            ...
            summary.textContent = entries.length + (entries.length === 1 ? " file" : " files") +
                ...
                (replacements ? " · " + replacements + " to overwrite" : "") +
                (done ? " · " + done + " added" : "") + (needsAttention ? " · " + needsAttention + " to review" : "");
            all.hidden = ready + replacements < 2 || checking > 0 || redirected;
            all.disabled = Boolean(active);
            all.textContent = (replacements ? "Add all & overwrite (" : "Add all (") + (ready + replacements) + ")";
            if (batchDescription) {
                batchDescription.hidden = all.hidden || !replacements;
                batchDescription.textContent = (ready ? "Adds " + ready + " new " + (ready === 1 ? "file" : "files") + " and overwrites " : "Overwrites ") +
                    replacements + " existing " + (replacements === 1 ? "file" : "files") + ". Files that need attention are skipped.";
            }
            hint.hidden = !active;
        ...
        function fail(entry, error) {
            if (error.redirecting) {
                ...
                present(entry, {
                    ...
                    label: error.canceled ? "Canceled" : error.blocked ? "Already uploaded" : "Could not add",
        ...
        async function check(entry) {
            ...
            try {
                ...
                if (result && result.blocked) {
                    ...
                    present(entry, {state: replaceable ? "replaceable" : "blocked", label: replaceable ? "Already uploaded · review overwrite" : "Cannot add", message: result.message, url: result.url, linkLabel: result.linkLabel, percent: null});
                    return false;
                }
                if (entry.overwriteKey !== null) {
                    entry.overwriteKey = null;
                    entry.availableOverwriteKey = null;
                    present(entry, {state: "selected", label: "Ready to add", message: "The original file is no longer present. Select Add to add this file."});
                    return false;
                }
                return true;
            } catch (error) {
        ...
        function select(files) {
            files.forEach(function(file) {
                ...
                var entry = {id: ++serial, file: file, key: key, state: "selected", label: "Ready to add", percent: null, message: "", meta: {}, attempted: false, duplicate: duplicate, overwriteKey: null, availableOverwriteKey: null};
                entries.push(entry);
                entry.row = ui.createFile(list, {
                    name: file.name, size: file.size, announcement: announcement,
                ...
                if (duplicate || validation) {
                    ...
                    check(entry).then(function(ready) {
                        if (ready && entries.indexOf(entry) !== -1) present(entry, {state: "selected", label: "Ready to add"});
                    });
                } else present(entry, {});
            });
            picker.input.value = "";
        ...
        function add(entry) {
            ...
            present(entry, {state: "queued", label: "Waiting to add", message: "", url: "", percent: null});
            pump();
        }
        function overwrite(entry) {
            ...
            present(entry, {state: "queued", label: "Waiting to overwrite", message: "", url: "", percent: null});
            pump();
        }
        async function pump() {
            ...
            if (await check(entry) && !redirected) {
                try {
                    ...
                    present(entry, {state: "uploading", label: "Adding…", message: "", percent: null});
        ...
        function addAll() {
            ...
            entries.filter(function(entry) { return entry.state === "selected" || entry.state === "replaceable"; }).forEach(function(entry) {
                ...
                present(entry, {state: "queued", label: entry.overwriteKey !== null ? "Waiting to overwrite" : "Waiting to add", message: ""});
            });
            pump();
        }
