...
on a product, then **Save specification**. Upload one reviewed JSON file,
...
does not duplicate that revision. The product and draft or unknown AOI scope are
...
stores immutable JSON documents in PostgreSQL and derives draft AOI scopes for
`/products`; `sync_product_catalog` applies a reviewed expected-delivery plan.
...
recipe files. Inactive state prevents archived products from falling back to a
